...
NetApp 不断改进和完善其产品和服务。以下是 Astra Trident 中的一些最新功能。对于先前版本，请参见 https://["文档的早期版本"]。
...
* 要对 iSCSI PV 执行联机空间回收，工作节点上的底层操作系统可能需要将挂载选项传递到卷。这一点适用于需要 `discard` 的 RHEL/RedHat CoreOS 实例 https://["挂载选项"^]；确保中包含 Discard mountOption https://["d4b9b9554fd820f43eae492d33e41167"^] 支持联机块丢弃。
