...
Astra Trident 21.07.0 * 不可下载 * 。对版本 21.07.0 中的 `snapshotReserve` 进行更改可能会导致 CSI `VolumeSnapshots` 无法用于创建 PersistentVolumeClaim 。

如果您已升级到 21.07.0 版，建议删除新创建的 `VolumeSnapshots` （在 21.07.0 版中配置）并降级到先前版本。
