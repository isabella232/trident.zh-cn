...
.步骤
. 如果您 link:../trident-get-started/kubernetes-deploy-helm.html#deploy-the-trident-operator-and-install-astra-trident-using-helm["已使用Helm安装Asta Trdent"]使用，则可以使用 `helm upgrade trident netapp-trident/trident-operator --version 100.2406.0` 一步升级。如果您未添加Helm repo或无法使用它进行升级：
+
