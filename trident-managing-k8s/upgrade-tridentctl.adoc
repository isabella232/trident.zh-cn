...
* 从 Trident 20.01 开始，仅限测试版 https://["卷快照"^] 受支持。Kubernetes 管理员应注意安全地备份或将 alpha Snapshot 对象转换为测试版，以保留原有的 alpha Snapshot 。
* 测试版的卷快照引入了一组经过修改的 CRD 和一个快照控制器，这两个控制器都应在安装 Astra Trident 之前进行设置。 https://["本博客"^] 讨论将 alpha 卷快照迁移到测试版格式所涉及的步骤。
