...
存储驱动程序的名称
 a| 
`ontap-nas` ， `ontap-san` ， `ontap-nas-economics` ， `ontap-nas-flexgroup` ， `solidfire-san` ， `azure-netapp-files` ， `AWS-CVS` 或 `GCP-CVS`
...
NOTE: NetApp Cloud Volumes Service for AWS 不支持小于 100 GB 的卷。为了便于部署应用程序，如果请求的卷较小， Trident 会自动创建 100 GB 的卷。
