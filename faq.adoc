...
如果您有任何问题，问题或请求，请通过我们的联系我们 http://netapp.io/slack["可宽延"^] 团队或 GitHub 。
...
从目前开始，我们为 `ontap-nas` ， `ontap-san` ， `ontap-san-economic` ， `solidfire-san` ， `AWS-CVS` 提供按需快照支持。 `GCP-CVS` 和 `azure-netapp-files` 后端驱动程序。
