...
NOTE: 如果您是首次使用Azure NetApp Files 或在新位置使用、则需要进行一些初始配置来设置Azure NetApp Files 并创建NFS卷。请参见 https://["Azure：设置Azure NetApp Files 并创建NFS卷"^]。

配置和使用 https://["Azure NetApp Files"^] 后端，您需要满足以下要求：
