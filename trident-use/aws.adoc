...
NOTE: 每个 `区域` 的 "apiURL"` 都是唯一的。 `例如， us-west-2 `apiRegion` 具有 https://cv.us-west-2.netapp.com:8080/v1/[] `apiURL` 。同样， us-east-1 `apiRegion` 也具有 `apiURL` 。请务必查看 CVS 信息板，了解后端配置的正确 `apiRegion` 和 `apiURL` 参数。
