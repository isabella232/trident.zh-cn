...
| `smbShare` | 您可以指定使用共享文件夹Microsoft管理控制台创建的SMB共享的名称、也可以指定共享名称以允许Astra Trident创建SMB共享。或者、您也可以将参数留空、以防止对卷进行通用共享访问。 | `smb-share` 
...
[%collapsible%open]
====
这是使用的基本配置 `ontap-nas-economy` 驱动程序。
...
----
====
.ONTAP NAS FlexGroup最低配置示例
[%collapsible%open]
====
这是使用的基本配置 `ontap-nas-flexgroup` 驱动程序。
...
[%collapsible%open]
====
此示例指示Astra Dandent使用动态导出策略自动创建和管理导出策略。此操作对于也是如此 `ontap-nas-economy` 和 `ontap-nas-flexgroup` 驱动程序。
...
[%collapsible%open]
====
此示例将使用SMB卷创建Amazon FSx for ONTAP后端。
