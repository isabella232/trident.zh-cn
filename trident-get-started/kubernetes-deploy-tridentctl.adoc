...
[role="lead"]
您可以使用安装Astra Trident `tridentctl`。此过程将在适用场景 中安装Trident和CSI映像 `docker.io` 和 `registry.k8s.io` 或私有的镜像注册表。以自定义 `tridentctl` 部署、请参见 link:kubernetes-customize-deploy-tridentctl.html["自定义 tridentctl 部署"]。
