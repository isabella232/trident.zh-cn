...
在中 `operator.yaml`、更新 `image: docker.io/netapp/trident-operator:23.01.0` 以反映映像注册表的位置。例如：。

* `image: <your-registry>/trident-operator:23.01.0` 如果您的映像全部位于一个注册表中。
* `image: <your-registry>/netapp/trident-operator:23.01.0` 如果Trident和CSI映像位于不同的注册表中。
...
=== 第4步：更新中的映像注册表位置 `TridentOrchestrator`

Trident和CSI映像可以位于一个注册表或不同的注册表中、但所有CSI映像都必须位于同一注册表中。更新 `deploy/crds/tridentorchestrator_cr.yaml` 根据注册表配置添加其他位置规格。
...
=== 第5步：创建 `TridentOrchestrator` 并安装Trident
